...
7. Add other variables:
   ...
   - `NODE_ENV`: `production`
   - `GOOGLE_CLIENT_ID`: Your Google OAuth Web Client ID (for "Sign in with Gmail")
...
- The button only renders when `GOOGLE_CLIENT_ID` is set on the server. Verify:
  - `GOOGLE_CLIENT_ID` is added to Render env vars
  - The env var was added **before** the last deploy (or triggered a new deploy)
- In Google Cloud Console → Credentials → your Web Client ID → **Authorized JavaScript origins** must include your exact app URL (e.g., `https://your-app.onrender.com`).
  - **Authorized redirect URIs are NOT required** for this app — it uses the Google Identity Services popup flow, not the server-side redirect flow.
- Test user: if your app is still in "Testing" status, the Google account you sign in with must be listed as a test user in the OAuth consent screen.
