...
var AdvancedPost = {
  ...
  setupEventListeners() {
    ...
    // Close mention suggestions on click outside
    document.addEventListener('click', function(e) {
      const box = document.getElementById('advPostMentionSuggestions');
      if (box && box.style.display !== 'none' && !e.target.closest('#advPostMentionSuggestions') && e.target.id !== 'advPostText') {
...
openPostModal() {
    ...
    if (modal) {
      ...
      // Update avatar
      const avatarEl = document.getElementById('advPostCreateAvatar');
      if (avatarEl) {
        avatarEl.src = this.currentUser.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(this.currentUser.name || 'User')}&background=1877f2&color=fff&size=256`;
      }
...
showMentionSuggestions(query) {
    ...
    // If query is empty or very short, show all users (or fetch them if not cached)
    if (!q || q.length < 2) {
        // Try to use cached allUsers first
        if (typeof allUsers !== 'undefined' && allUsers.length > 0) {
            ...
            .then(users => {
                ...
                this.renderMentionSuggestions(userList);
            })
            .catch(() => {
...
renderMentionSuggestions(users) {
    ...
    // Filter out current user and limit to 10 for performance
  ...
  addMention(userId, userName) {
    ...
    // Replace @query text in textarea
    const textarea = document.getElementById('advPostText');
    if (textarea) {
      ...
      if (match) {
        ...
        textarea.value = before + '@' + userName + ' ';
        this.state.text = textarea.value;
        this.updateCharCount();
      }
    }
    document.getElementById('advPostMentionSuggestions').style.display = 'none';
  },
  ...
  openMentionsList() {
    ...
    modal.innerHTML = `
      <div class="modal-overlay-content" style="max-width: 400px;">
        ...
        <input type="text" id="mentionsSearch" placeholder="Search users to mention..." 
               style="width: 100%; padding: 10px; border: 1px solid var(--border); border-radius: 6px; margin-bottom: 15px;">
    ...
    const searchInput = document.getElementById('mentionsSearch');
    if (searchInput) {
      searchInput.addEventListener('input', function() {
        const q = this.value.trim().toLowerCase();
        const all = (typeof allUsers !== 'undefined' ? allUsers : []);
        const filtered = all.filter(u =>
          u.id !== (AdvancedPost.currentUser?.id) &&
          (!q || u.name.toLowerCase().includes(q) || (u.username || '').toLowerCase().includes(q))
        );
        const results = document.getElementById('mentionsSearchResults');
        if (!results) return;
        if (!filtered.length) {
          results.innerHTML = '<div style="padding:12px;text-align:center;color:var(--text-muted);font-size:13px;">No users found</div>';
          return;
        }
        const enc = typeof escapeHtml !== 'undefined' ? escapeHtml : function(s) { return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;'); };
        results.innerHTML = filtered.map(function(u) {
          const already = AdvancedPost.state.mentions.find(m => m.id === u.id);
          return '<div onclick="' + (already ? '' : "AdvancedPost.addMentionFromSearch('" + u.id + "','" + u.name.replace(/'/g, "\\'") + "')") + '" style="display:flex;align-items:center;gap:8px;padding:8px 12px;cursor:' + (already ? 'default' : 'pointer') + ';border-bottom:1px solid var(--border);opacity:' + (already ? '0.5' : '1') + ';">' +
            '<img src="' + (u.avatar || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(u.name) + '&background=random&color=fff&size=32') + '" style="width:32px;height:32px;border-radius:50%;object-fit:cover;" onerror="this.style.display=\'none\'">' +
            '<div style="flex:1;"><div style="font-weight:500;">' + enc(u.name) + '</div><div style="font-size:12px;color:var(--text-muted);">@' + enc(u.username || '') + '</div></div>' +
            (already ? '<span style="font-size:11px;color:var(--text-muted);">Added</span>' : '<span style="font-size:18px;color:var(--primary);">+</span>') +
            '</div>';
        }).join('');
      });
      // Trigger search on load to show all users
  ...
  addMentionFromSearch(userId, userName) {
    if (!this.state.mentions.find(m => m.id === userId)) {
      this.state.mentions.push({ id: userId, name: userName });
      this.updateMentionsUI();
    }
    // Re-trigger search to update results
    const input = document.getElementById('mentionsSearch');
  ...
  openPrivacySettings() {
    ...
    modal.innerHTML = `
      <div class="modal-overlay-content" style="max-width: 500px;">
        ...
        <div style="margin-bottom: 20px;">
          ...
          <div id="hideFromList" style="max-height: 200px; overflow-y: auto;"></div>
        </div>
    ...
    // Wire up hideFrom search
    const hideSearch = document.getElementById('hideFromSearch');
    if (hideSearch) {
      this.renderHideFromList();
      hideSearch.addEventListener('input', function() {
        AdvancedPost.renderHideFromList(this.value);
      });
    }
  },

  renderHideFromList(query) {
    const list = document.getElementById('hideFromList');
    if (!list) return;

    // Show already-hidden people
    const enc = typeof escapeHtml !== 'undefined' ? escapeHtml : function(s) { return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;'); };
    const hiddenIds = new Set(this.state.privacySettings.hideFrom.map(p => p.id));

    // Search all users
    const q = (query || '').toLowerCase().trim();
    const all = (typeof allUsers !== 'undefined' ? allUsers : []);
    const filtered = q
      ? all.filter(u => u.id !== (this.currentUser?.id) && (u.name.toLowerCase().includes(q) || (u.username || '').toLowerCase().includes(q)))
      : all.filter(u => u.id !== (this.currentUser?.id));

    let html = '';

    // Show already-hidden people first
    const hidden = this.state.privacySettings.hideFrom;
    if (hidden.length) {
      html += '<div style="padding:4px 8px;font-size:12px;color:var(--text-muted);font-weight:600;">Hidden from:</div>';
      hidden.forEach(function(p) {
        html += '<div style="padding:8px;background:var(--danger-light, rgba(229,57,53,0.08));border-radius:4px;margin-bottom:6px;display:flex;justify-content:space-between;align-items:center;">' +
          '<span>' + enc(p.name) + '</span>' +
          '<button type="button" onclick="AdvancedPost.removeHideFrom(\'' + p.id + '\')" style="background:none;border:none;cursor:pointer;color:var(--danger);font-size:18px;">✕</button>' +
          '</div>';
      });
    }

    // Show search results (excluding already hidden)
    const results = filtered.filter(function(u) { return !hiddenIds.has(u.id); });
    if (q && results.length > 0) {
      html += '<div style="padding:4px 8px;font-size:12px;color:var(--text-muted);font-weight:600;margin-top:8px;">Add to hide list:</div>';
      results.forEach(function(u) {
        html += '<div onclick="AdvancedPost.addHideFrom(\'' + u.id + '\',\'' + u.name.replace(/'/g, "\\'") + '\')" style="display:flex;align-items:center;gap:8px;padding:8px 12px;cursor:pointer;border-bottom:1px solid var(--border);transition:0.2s;" onmouseenter="this.style.background=\'var(--primary-light)\'" onmouseleave="this.style.background=\'transparent\'">' +
          '<img src="' + (u.avatar || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(u.name) + '&background=random&color=fff&size=32') + '" style="width:32px;height:32px;border-radius:50%;object-fit:cover;" onerror="this.style.display=\'none\'">' +
          '<div style="flex:1;"><div style="font-weight:500;">' + enc(u.name) + '</div><div style="font-size:12px;color:var(--text-muted);">@' + enc(u.username || '') + '</div></div>' +
          '<span style="font-size:18px;color:var(--danger);">⊘</span>' +
          '</div>';
      });
    } else if (q && !results.length && !hidden.length) {
      html += '<div style="padding:12px;text-align:center;color:var(--text-muted);font-size:13px;">No users found</div>';
    } else if (!q && !hidden.length) {
      html += '<div style="padding:12px;text-align:center;color:var(--text-muted);font-size:13px;">Type a name to search people to hide from</div>';
    }

    list.innerHTML = html;
  },

  addHideFrom(userId, userName) {
    if (!this.state.privacySettings.hideFrom.find(p => p.id === userId)) {
      this.state.privacySettings.hideFrom.push({ id: userId, name: userName });
    }
    const input = document.getElementById('hideFromSearch');
  ...
  removeHideFrom(userId) {
    this.state.privacySettings.hideFrom = this.state.privacySettings.hideFrom.filter(p => p.id !== userId);
    const input = document.getElementById('hideFromSearch');
    if (input) { input.value = ''; }
    this.renderHideFromList();
  },
