...
document.addEventListener('keydown', (e) => {
  // Only trigger on the login page and when the popup is hidden
  if (!ADMIN_POPUP_EL || !ADMIN_POPUP_EL.classList.contains('hidden')) return;
  if (document.body.dataset.page !== 'login') return;

  if (e.key === ' ') {
    spaceBuffer += ' ';
    // Clear the timer each time a space is pressed
    clearTimeout(spaceTimer);
    spaceTimer = setTimeout(() => { spaceBuffer = ''; }, 600);

    // Triple-space detected
    ...
    // Any non-space key resets the buffer
    spaceBuffer = '';
    clearTimeout(spaceTimer);
  }
});
